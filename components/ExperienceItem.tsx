...
const ExperienceItem: React.FC<ExperienceItemProps> = ({
  ...
  return (
    <motion.div
      ...
      onHoverStart={(e) => {}}
      onHoverEnd={(e) => {}}
      className="flex flex-col w-full shadow-xl border-transparent rounded-md justify-center items-start bg-opacity-30 bg-white mb-4 cursor-pointer hover:opacity-80 text-black dark:text-white"
    >
      <h1 className="p-2">{title}</h1>
      <div
        className={`flex flex-row w-full justify-between items-center px-2 font-semibold font-sans ${colorStyle}`}
      >
        <p>{company}</p>
        <p>
          {from} - {until}
